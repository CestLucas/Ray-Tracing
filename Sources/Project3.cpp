...
int main(int argc, char **argv)
{
	...
	while (running)
	{
		...
		{
			//get the direction we are looking
			glm::vec3 gaze = glm::normalize(lookAt - eye);
			//cross up and dir to get a vector to our left
			glm::vec3 left = glm::normalize(glm::cross(gaze, up));

			//view variables
			float asp_ratio = float(SCR_WIDTH) / SCR_HEIGHT;
			float fov = tan(fovy / 2);
			float inverse_width = 1 / float(SCR_WIDTH);
			float inverse_height = 1 / float(SCR_HEIGHT);
			...
			for (int y = 0; y < SCR_HEIGHT; y++)
			{
				...
				// can't get openMP working right so just commented it out
				for (int x = 0; x < SCR_WIDTH; x++)
				{
					glm::vec3 currentColor;
					...
					//formula from the text + raytracerIntro.pdf
					float u = asp_ratio * fov * (2 * ((x + 0.5f) * inverse_width) - 1);
					float v = fov * (1 - 2 * ((y + 0.5f) * inverse_height));

					glm::vec3 currentDir = glm::normalize(gaze + u * left + v * up);

					currentColor = myScene->rayTrace(eye, currentDir, 0); // initial recursive depth is 0
